...
## DB / API compatibility risks
- Schema/view compatibility: the current API assumes the database exposes the workflow functions, `app_user` execute grants, and `workflow.v_*` read views documented here. API code that calls new workflow functions or depends on new view columns is not backward-compatible with an older schema snapshot that lacks those objects.
- Older-client impact: older clients may break if they relied on removed or narrowed contracts, especially `recipient_user_id` notification inbox override, generic commented-file delete without owner/superuser authorization, or revision lists implicitly returning canceled/superseded rows by default.
