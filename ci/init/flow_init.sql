-- ========================================================
-- POSTGRESQL MIGRATION SCRIPT FOR FLOW (FINAL)
-- ========================================================
-- 1. Setup Environment
--    We use a dedicated schema to keep the workspace clean.
-- ========================================================

CREATE SCHEMA IF NOT EXISTS flow;
SET search_path TO flow;

-- ========================================================
-- 2. Reference / Lookup Tables
-- ========================================================

CREATE TABLE areas (
    area_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    area_name VARCHAR(45) NOT NULL UNIQUE,
    area_acronym VARCHAR(5) NOT NULL UNIQUE
);

CREATE TABLE disciplines (
    discipline_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    discipline_name VARCHAR(45) NOT NULL UNIQUE,
    discipline_acronym VARCHAR(5) NOT NULL UNIQUE
);

CREATE TABLE projects (
    project_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    project_name VARCHAR(45) NOT NULL UNIQUE
);

CREATE TABLE units (
    unit_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    unit_name VARCHAR(45) NOT NULL UNIQUE
);

CREATE TABLE jobpacks (
    jobpack_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    jobpack_name VARCHAR(45) NOT NULL UNIQUE
);

CREATE TABLE roles (
    role_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    role_name VARCHAR(45) NOT NULL UNIQUE
);

CREATE TABLE doc_rev_milestones (
    milestone_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    milestone_name VARCHAR(45) NOT NULL UNIQUE,
    progress SMALLINT DEFAULT 0
);

CREATE TABLE revision_overview (
    rev_code_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    rev_code_name VARCHAR(15) NOT NULL UNIQUE,
    rev_code_acronym VARCHAR(5) NOT NULL,
    rev_description VARCHAR(45) NOT NULL UNIQUE,
    percentage SMALLINT
);

CREATE TABLE doc_rev_status_ui_behaviors (
    ui_behavior_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ui_behavior_name VARCHAR(64) NOT NULL UNIQUE,
    ui_behavior_file VARCHAR(64) NOT NULL UNIQUE
);

CREATE TABLE doc_rev_statuses (
    rev_status_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    rev_status_name VARCHAR(45) NOT NULL UNIQUE,
    ui_behavior_id SMALLINT NOT NULL REFERENCES flow.doc_rev_status_ui_behaviors(ui_behavior_id),
    next_rev_status_id SMALLINT REFERENCES flow.doc_rev_statuses(rev_status_id),
    revertible BOOLEAN NOT NULL DEFAULT TRUE,
    editable BOOLEAN NOT NULL DEFAULT TRUE,
    final BOOLEAN NOT NULL DEFAULT FALSE,
    start BOOLEAN NOT NULL DEFAULT FALSE,
    CONSTRAINT chk_doc_rev_statuses_no_self_ref CHECK (
        next_rev_status_id IS NULL OR next_rev_status_id <> rev_status_id
    ),
    CONSTRAINT chk_doc_rev_statuses_final_flags CHECK (
        NOT final OR (editable = FALSE AND revertible = FALSE)
    ),
    CONSTRAINT chk_doc_rev_statuses_final_next_eq CHECK (
        final = (next_rev_status_id IS NULL)
    )
);

-- Enforce a single global "final" status:
-- - chk_doc_rev_statuses_final_next_eq ties final = (next_rev_status_id IS NULL)
-- - This partial unique index ensures only one row can have next_rev_status_id IS NULL,
--   and therefore only one final status can exist in the entire system.
-- Application / API logic must be aware that creating or updating a second final
-- status will fail due to this constraint.
CREATE UNIQUE INDEX ux_doc_rev_statuses_single_final
    ON flow.doc_rev_statuses ((next_rev_status_id IS NULL))
    WHERE next_rev_status_id IS NULL;

CREATE UNIQUE INDEX ux_doc_rev_statuses_single_start
    ON flow.doc_rev_statuses (start)
    WHERE start = TRUE;

COMMENT ON TABLE doc_rev_statuses IS
    'Global workflow statuses. Business rule: only one status may be final (next_rev_status_id IS NULL) and only one may be start.';
COMMENT ON COLUMN doc_rev_statuses.final IS
    'Marks the single final status in the global workflow (only one allowed).';
COMMENT ON COLUMN doc_rev_statuses.start IS
    'Marks the single start status in the global workflow (only one allowed).';
COMMENT ON INDEX ux_doc_rev_statuses_single_final IS
    'Enforces single final status in the global workflow.';
COMMENT ON INDEX ux_doc_rev_statuses_single_start IS
    'Enforces single start status in the global workflow.';

CREATE OR REPLACE FUNCTION doc_rev_statuses_prevent_cycle()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
    current_id SMALLINT;
BEGIN
    IF NEW.next_rev_status_id IS NULL THEN
        RETURN NEW;
    END IF;

    current_id := NEW.next_rev_status_id;
    WHILE current_id IS NOT NULL LOOP
        IF NEW.rev_status_id IS NOT NULL AND current_id = NEW.rev_status_id THEN
            RAISE EXCEPTION 'Cycle detected in doc_rev_statuses';
        END IF;

        SELECT next_rev_status_id
        INTO current_id
        FROM flow.doc_rev_statuses
        WHERE rev_status_id = current_id;
    END LOOP;

    RETURN NEW;
END;
$$;

CREATE TRIGGER trg_doc_rev_statuses_prevent_cycle
BEFORE INSERT OR UPDATE OF next_rev_status_id
ON flow.doc_rev_statuses
FOR EACH ROW
EXECUTE FUNCTION doc_rev_statuses_prevent_cycle();

CREATE TABLE files_accepted (
    file_type VARCHAR(10) PRIMARY KEY,
    mimetype VARCHAR(90) NOT NULL
);

CREATE TABLE leased_doc_nums (
    doc_number VARCHAR(45) NOT NULL,
    created_date TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);



-- ========================================================
-- 3. Relation & User Tables
-- ========================================================

CREATE TABLE person (
    person_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    person_name VARCHAR(45) NOT NULL,
    photo_s3_uid TEXT
);

CREATE TABLE users (
    user_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    person_id SMALLINT NOT NULL REFERENCES flow.person(person_id),
    user_acronym VARCHAR(10) NOT NULL,
    role_id SMALLINT NOT NULL REFERENCES flow.roles(role_id),
    UNIQUE (person_id)
);

CREATE TABLE doc_types (
    type_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    doc_type_name VARCHAR(45) NOT NULL,
    ref_discipline_id SMALLINT NOT NULL REFERENCES flow.disciplines(discipline_id),
    doc_type_acronym VARCHAR(10) NOT NULL
);

CREATE TABLE distribution_list (
    dist_id SMALLINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    distribution_list_name VARCHAR(45) NOT NULL,
    project_id SMALLINT NOT NULL REFERENCES flow.projects(project_id)
);

CREATE TABLE distribution_list_content (
    dist_id SMALLINT NOT NULL REFERENCES flow.distribution_list(dist_id),
    person_id SMALLINT NOT NULL REFERENCES flow.person(person_id),
    PRIMARY KEY (dist_id, person_id)
);

CREATE TABLE permissions (
    permission_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id SMALLINT NOT NULL REFERENCES flow.users(user_id),
    project_id SMALLINT REFERENCES flow.projects(project_id),
    discipline_id SMALLINT REFERENCES flow.disciplines(discipline_id),
    CONSTRAINT chk_permissions_scope CHECK (project_id IS NOT NULL OR discipline_id IS NOT NULL)
);

CREATE UNIQUE INDEX uq_permissions_user_proj_disc
    ON flow.permissions(user_id, project_id, discipline_id)
    WHERE project_id IS NOT NULL AND discipline_id IS NOT NULL;

CREATE UNIQUE INDEX uq_permissions_user_proj_anydisc
    ON flow.permissions(user_id, project_id)
    WHERE discipline_id IS NULL;

CREATE UNIQUE INDEX uq_permissions_user_anyproj_disc
    ON flow.permissions(user_id, discipline_id)
    WHERE project_id IS NULL;

CREATE UNIQUE INDEX uq_permissions_user_anyscope
    ON flow.permissions(user_id)
    WHERE project_id IS NULL AND discipline_id IS NULL;

-- ========================================================
-- 4. Core Documentation Tables
-- ========================================================

CREATE TABLE doc (
    doc_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    doc_name_unique VARCHAR(45) NOT NULL UNIQUE,
    title VARCHAR(45) NOT NULL,
    project_id SMALLINT REFERENCES flow.projects(project_id),
    jobpack_id SMALLINT REFERENCES flow.jobpacks(jobpack_id),
    type_id SMALLINT NOT NULL REFERENCES flow.doc_types(type_id),
    area_id SMALLINT NOT NULL REFERENCES flow.areas(area_id),
    unit_id SMALLINT NOT NULL REFERENCES flow.units(unit_id),
    rev_actual_id INTEGER,
    rev_current_id INTEGER,
    voided BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    created_by SMALLINT REFERENCES flow.users(user_id),
    updated_by SMALLINT REFERENCES flow.users(user_id)
);

CREATE INDEX idx_doc_voided ON flow.doc (voided);

CREATE TABLE doc_cache (
    user_id SMALLINT NOT NULL,
    project_id SMALLINT NOT NULL REFERENCES flow.projects(project_id),
    doc_name_unique VARCHAR(45) NOT NULL UNIQUE,
    title VARCHAR(45) NOT NULL,
    jobpack_id SMALLINT REFERENCES flow.jobpacks(jobpack_id),
    type_id SMALLINT NOT NULL REFERENCES flow.doc_types(type_id),
    area_id SMALLINT NOT NULL REFERENCES flow.areas(area_id),
    unit_id SMALLINT NOT NULL REFERENCES flow.units(unit_id),
    PRIMARY KEY (user_id, project_id, doc_name_unique)
);

CREATE TABLE doc_revision (
    rev_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    rev_code_id SMALLINT NOT NULL REFERENCES flow.revision_overview(rev_code_id),
    rev_date TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    rev_author_id SMALLINT NOT NULL REFERENCES flow.person(person_id),
    rev_originator_id SMALLINT NOT NULL REFERENCES flow.person(person_id),
    as_built BOOLEAN NOT NULL DEFAULT FALSE,
    superseded BOOLEAN NOT NULL DEFAULT FALSE,
    voided BOOLEAN NOT NULL DEFAULT FALSE,
    transmital_current_revision VARCHAR(45) NOT NULL,
    milestone_id SMALLINT REFERENCES flow.doc_rev_milestones(milestone_id),
    planned_start_date TIMESTAMPTZ NOT NULL,
    planned_finish_date TIMESTAMPTZ NOT NULL,
    actual_start_date TIMESTAMPTZ,
    actual_finish_date TIMESTAMPTZ,
    canceled_date TIMESTAMPTZ,
    rev_status_id SMALLINT NOT NULL REFERENCES flow.doc_rev_statuses(rev_status_id),
    doc_id INTEGER NOT NULL REFERENCES flow.doc(doc_id) ON DELETE CASCADE,
    seq_num SMALLINT NOT NULL DEFAULT 1,
    rev_modifier_id SMALLINT NOT NULL REFERENCES flow.person(person_id),
    modified_doc_date TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    created_by SMALLINT REFERENCES flow.users(user_id),
    updated_by SMALLINT REFERENCES flow.users(user_id)
);

ALTER TABLE doc 
    ADD CONSTRAINT fk_doc_rev_current FOREIGN KEY (rev_current_id) REFERENCES flow.doc_revision(rev_id),
    ADD CONSTRAINT fk_doc_rev_actual FOREIGN KEY (rev_actual_id) REFERENCES flow.doc_revision(rev_id);

CREATE TABLE doc_revision_history (
    rev_id INTEGER NOT NULL,
    rev_code_id SMALLINT NOT NULL,
    rev_date TIMESTAMPTZ NOT NULL,
    rev_author_id SMALLINT NOT NULL,
    rev_originator_id SMALLINT NOT NULL,
    as_built BOOLEAN NOT NULL DEFAULT FALSE,
    superseded BOOLEAN NOT NULL DEFAULT FALSE,
    voided BOOLEAN NOT NULL DEFAULT FALSE,
    transmital_current_revision VARCHAR(45) NOT NULL,
    milestone_id SMALLINT,
    planned_start_date TIMESTAMPTZ NOT NULL,
    planned_finish_date TIMESTAMPTZ NOT NULL,
    actual_start_date TIMESTAMPTZ,
    actual_finish_date TIMESTAMPTZ,
    canceled_date TIMESTAMPTZ,
    rev_status_id SMALLINT NOT NULL,
    doc_id INTEGER NOT NULL,
    seq_num SMALLINT NOT NULL,
    archived_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);

CREATE OR REPLACE FUNCTION flow.create_doc_with_revision(
    p_doc_name_unique VARCHAR(45),
    p_title VARCHAR(45),
    p_project_id SMALLINT,
    p_jobpack_id SMALLINT,
    p_type_id SMALLINT,
    p_area_id SMALLINT,
    p_unit_id SMALLINT,
    p_rev_code_id SMALLINT,
    p_rev_date TIMESTAMPTZ,
    p_rev_author_id SMALLINT,
    p_rev_originator_id SMALLINT,
    p_rev_modifier_id SMALLINT,
    p_transmital_current_revision VARCHAR(45),
    p_milestone_id SMALLINT,
    p_planned_start_date TIMESTAMPTZ,
    p_planned_finish_date TIMESTAMPTZ,
    p_actual_start_date TIMESTAMPTZ,
    p_actual_finish_date TIMESTAMPTZ,
    p_canceled_date TIMESTAMPTZ,
    p_modified_doc_date TIMESTAMPTZ
) RETURNS TABLE (out_doc_id INTEGER, out_rev_id INTEGER)
LANGUAGE plpgsql
AS $$
DECLARE
    v_start_status SMALLINT;
    v_doc_id INTEGER;
    v_rev_id INTEGER;
BEGIN
    SELECT rev_status_id
    INTO v_start_status
    FROM flow.doc_rev_statuses
    WHERE start = TRUE
    LIMIT 1;

    IF v_start_status IS NULL THEN
        RAISE EXCEPTION 'No start status found in doc_rev_statuses';
    END IF;

    INSERT INTO flow.doc (
        doc_name_unique,
        title,
        project_id,
        jobpack_id,
        type_id,
        area_id,
        unit_id
    ) VALUES (
        p_doc_name_unique,
        p_title,
        p_project_id,
        p_jobpack_id,
        p_type_id,
        p_area_id,
        p_unit_id
    )
    RETURNING doc_id INTO v_doc_id;

    INSERT INTO flow.doc_revision (
        rev_code_id,
        rev_date,
        rev_author_id,
        rev_originator_id,
        transmital_current_revision,
        milestone_id,
        planned_start_date,
        planned_finish_date,
        actual_start_date,
        actual_finish_date,
        canceled_date,
        rev_status_id,
        doc_id,
        seq_num,
        rev_modifier_id,
        modified_doc_date
    ) VALUES (
        p_rev_code_id,
        COALESCE(p_rev_date, CURRENT_TIMESTAMP),
        p_rev_author_id,
        p_rev_originator_id,
        p_transmital_current_revision,
        p_milestone_id,
        p_planned_start_date,
        p_planned_finish_date,
        p_actual_start_date,
        p_actual_finish_date,
        p_canceled_date,
        v_start_status,
        v_doc_id,
        1,
        p_rev_modifier_id,
        COALESCE(p_modified_doc_date, CURRENT_TIMESTAMP)
    )
    RETURNING rev_id INTO v_rev_id;

    UPDATE flow.doc
    SET rev_current_id = v_rev_id
    WHERE doc_id = v_doc_id;

    RETURN QUERY SELECT v_doc_id, v_rev_id;
END;
$$;

CREATE TABLE files (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    filename VARCHAR(90) NOT NULL,
    s3_uid TEXT NOT NULL,
    mimetype VARCHAR(90) NOT NULL,
    rev_id INTEGER NOT NULL REFERENCES flow.doc_revision(rev_id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    created_by SMALLINT REFERENCES flow.users(user_id),
    updated_by SMALLINT REFERENCES flow.users(user_id)
);

CREATE TABLE files_commented (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    file_id INTEGER NOT NULL REFERENCES flow.files(id) ON DELETE CASCADE,
    user_id SMALLINT NOT NULL REFERENCES flow.users(user_id),
    s3_uid TEXT NOT NULL,
    mimetype VARCHAR(90) NOT NULL,
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP NOT NULL,
    created_by SMALLINT REFERENCES flow.users(user_id),
    updated_by SMALLINT REFERENCES flow.users(user_id),
    UNIQUE(file_id, user_id)
);

CREATE INDEX idx_files_commented_file_id ON flow.files_commented (file_id);

-- ========================================================
-- 5. Views
-- ========================================================

CREATE OR REPLACE VIEW flow.doc_revision_history_view AS
    SELECT 
        rev_id, rev_code_id, rev_date, rev_author_id, rev_originator_id,
        as_built, superseded, voided, 
        transmital_current_revision, 
        milestone_id,
        planned_start_date, planned_finish_date, actual_start_date, actual_finish_date,
        canceled_date, rev_status_id, doc_id, seq_num, 
        'CURRENT' as source_type
    FROM flow.doc_revision 
    UNION ALL 
    SELECT 
        rev_id, rev_code_id, rev_date, rev_author_id, rev_originator_id,
        as_built, superseded, voided, 
        transmital_current_revision, 
        milestone_id,
        planned_start_date, planned_finish_date, actual_start_date, actual_finish_date,
        canceled_date, rev_status_id, doc_id, seq_num,
        'HISTORY' as source_type
    FROM flow.doc_revision_history;

-- ========================================================
-- 6. Business Logic (Triggers)
-- ========================================================

CREATE OR REPLACE FUNCTION fn_doc_revision_after_insert() RETURNS TRIGGER AS $$
BEGIN
    UPDATE flow.doc SET rev_current_id = NEW.rev_id WHERE doc_id = NEW.doc_id;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER tr_doc_revision_after_insert
AFTER INSERT ON flow.doc_revision
FOR EACH ROW EXECUTE FUNCTION fn_doc_revision_after_insert();

CREATE OR REPLACE FUNCTION fn_doc_revision_after_update() RETURNS TRIGGER AS $$
BEGIN
    IF NEW.rev_code_id <> OLD.rev_code_id THEN
        INSERT INTO flow.doc_revision_history (
            rev_id, rev_code_id, rev_date, rev_author_id, rev_originator_id, 
            as_built, superseded, voided, 
            transmital_current_revision, 
            milestone_id, 
            planned_start_date, planned_finish_date, actual_start_date, actual_finish_date, 
            canceled_date, rev_status_id, doc_id, seq_num
        ) VALUES (
            OLD.rev_id, OLD.rev_code_id, OLD.rev_date, OLD.rev_author_id, OLD.rev_originator_id, 
            OLD.as_built, OLD.superseded, OLD.voided, 
            OLD.transmital_current_revision, 
            OLD.milestone_id, 
            OLD.planned_start_date, OLD.planned_finish_date, OLD.actual_start_date, 
            NOW(),
            OLD.canceled_date, OLD.rev_status_id, OLD.doc_id, OLD.seq_num
        );
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER tr_doc_revision_after_update
AFTER UPDATE ON flow.doc_revision
FOR EACH ROW EXECUTE FUNCTION fn_doc_revision_after_update();

CREATE OR REPLACE FUNCTION fn_doc_revision_before_delete() RETURNS TRIGGER AS $$
BEGIN
    DELETE FROM flow.files_commented
    WHERE file_id IN (SELECT id FROM flow.files WHERE rev_id = OLD.rev_id);
    DELETE FROM flow.files WHERE rev_id = OLD.rev_id;
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER tr_doc_revision_before_delete
BEFORE DELETE ON flow.doc_revision
FOR EACH ROW EXECUTE FUNCTION fn_doc_revision_before_delete();

CREATE OR REPLACE FUNCTION fn_doc_revision_after_delete() RETURNS TRIGGER AS $$
BEGIN
    UPDATE flow.doc SET rev_current_id = rev_actual_id WHERE doc_id = OLD.doc_id;
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER tr_doc_revision_after_delete
AFTER DELETE ON flow.doc_revision
FOR EACH ROW EXECUTE FUNCTION fn_doc_revision_after_delete();

CREATE OR REPLACE FUNCTION fn_files_commented_check_mimetype() RETURNS TRIGGER AS $$
DECLARE
    original_mimetype VARCHAR(90);
BEGIN
    SELECT mimetype INTO original_mimetype FROM flow.files WHERE id = NEW.file_id;
    IF original_mimetype IS NULL THEN
        RAISE EXCEPTION 'File not found for commented file_id=%', NEW.file_id
            USING ERRCODE = '23503';
    END IF;
    IF NEW.mimetype IS NULL OR lower(NEW.mimetype) <> lower(original_mimetype) THEN
        RAISE EXCEPTION 'Commented file mimetype % does not match original %',
            NEW.mimetype, original_mimetype
            USING ERRCODE = '23514';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER tr_files_commented_check_mimetype
BEFORE INSERT OR UPDATE ON flow.files_commented
FOR EACH ROW EXECUTE FUNCTION fn_files_commented_check_mimetype();

CREATE OR REPLACE FUNCTION fn_doc_revision_set_start_status() RETURNS TRIGGER AS $$
DECLARE
    v_start_status SMALLINT;
BEGIN
    IF NEW.rev_status_id IS NULL THEN
        SELECT rev_status_id
        INTO v_start_status
        FROM flow.doc_rev_statuses
        WHERE start = TRUE
        LIMIT 1;
        IF v_start_status IS NULL THEN
            RAISE EXCEPTION 'No start status found in doc_rev_statuses';
        END IF;
        NEW.rev_status_id = v_start_status;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER tr_doc_revision_set_start_status
BEFORE INSERT ON flow.doc_revision
FOR EACH ROW EXECUTE FUNCTION fn_doc_revision_set_start_status();

CREATE OR REPLACE FUNCTION doc_revision_status_transform(
    p_rev_id INTEGER,
    p_direction TEXT
) RETURNS SMALLINT
LANGUAGE plpgsql
AS $$
DECLARE
    current_status_id SMALLINT;
    next_status_id SMALLINT;
    current_status RECORD;
BEGIN
    IF p_direction NOT IN ('forward', 'back') THEN
        RAISE EXCEPTION 'Invalid direction: %', p_direction;
    END IF;

    SELECT rev_status_id
    INTO current_status_id
    FROM flow.doc_revision
    WHERE rev_id = p_rev_id;

    IF current_status_id IS NULL THEN
        RAISE EXCEPTION 'Revision not found';
    END IF;

    SELECT rev_status_id, next_rev_status_id, revertible, final, start
    INTO current_status
    FROM flow.doc_rev_statuses
    WHERE rev_status_id = current_status_id;

    IF p_direction = 'forward' THEN
        IF current_status.final OR current_status.next_rev_status_id IS NULL THEN
            RAISE EXCEPTION 'Revision already at final status';
        END IF;
        next_status_id := current_status.next_rev_status_id;
    ELSE
        IF current_status.start THEN
            RAISE EXCEPTION 'Revision already at start status';
        END IF;
        IF NOT current_status.revertible THEN
            RAISE EXCEPTION 'Revision status not revertible';
        END IF;
        SELECT rev_status_id
        INTO next_status_id
        FROM flow.doc_rev_statuses
        WHERE next_rev_status_id = current_status.rev_status_id;
        IF next_status_id IS NULL THEN
            RAISE EXCEPTION 'Previous status not found';
        END IF;
    END IF;

    UPDATE flow.doc_revision
    SET rev_status_id = next_status_id,
        updated_at = NULL,
        updated_by = NULL
    WHERE rev_id = p_rev_id;

    RETURN next_status_id;
END;
$$;

-- ========================================================
-- 7. Audit Triggers for updated_at
-- ========================================================

CREATE OR REPLACE FUNCTION fn_update_timestamp() RETURNS TRIGGER AS $$
BEGIN
    IF NEW.updated_at IS NULL THEN
        NEW.updated_at = CURRENT_TIMESTAMP;
    END IF;
    IF NEW.updated_by IS NULL THEN
        NEW.updated_by = NULLIF(current_setting('app.user', true), '')::SMALLINT;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION fn_set_created_by() RETURNS TRIGGER AS $$
BEGIN
    IF NEW.created_by IS NULL THEN
        NEW.created_by = NULLIF(current_setting('app.user', true), '')::SMALLINT;
    END IF;
    IF NEW.updated_by IS NULL THEN
        NEW.updated_by = NULLIF(current_setting('app.user', true), '')::SMALLINT;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER tr_doc_update_timestamp
BEFORE UPDATE ON flow.doc
FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp();

CREATE TRIGGER tr_doc_set_created_by
BEFORE INSERT ON flow.doc
FOR EACH ROW EXECUTE FUNCTION fn_set_created_by();

CREATE TRIGGER tr_doc_revision_update_timestamp
BEFORE UPDATE ON flow.doc_revision
FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp();

CREATE TRIGGER tr_doc_revision_set_created_by
BEFORE INSERT ON flow.doc_revision
FOR EACH ROW EXECUTE FUNCTION fn_set_created_by();

CREATE TRIGGER tr_files_update_timestamp
BEFORE UPDATE ON flow.files
FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp();

CREATE TRIGGER tr_files_set_created_by
BEFORE INSERT ON flow.files
FOR EACH ROW EXECUTE FUNCTION fn_set_created_by();

CREATE TRIGGER tr_files_commented_update_timestamp
BEFORE UPDATE ON flow.files_commented
FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp();

CREATE TRIGGER tr_files_commented_set_created_by
BEFORE INSERT ON flow.files_commented
FOR EACH ROW EXECUTE FUNCTION fn_set_created_by();
