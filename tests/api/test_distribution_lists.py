...
def test_endpoints():
    ...
    create_response = make_request(
        "POST",
        f"/documents/{DOC_ID}/distribution-lists",
        {"name": "Review Team"}
    )
    ...
    get_lists_response = make_request(
        "GET",
        f"/documents/{DOC_ID}/distribution-lists"
    )
    ...
    if add_recipient_response["status"] == 200:
    ...
    if create_response2["status"] == 200:
        ...
        delete_response = make_request(
            "DELETE",
            f"/documents/{DOC_ID}/distribution-lists/{temp_list_id}"
        )
        ...
        print_error(f"Failed to create temporary list")
        return
    
    # Test 7: Verify deletion
    print_test("Test 7: Verify List Count After Deletion")
    get_lists_response2 = make_request(
        "GET",
        f"/documents/{DOC_ID}/distribution-lists"
    )
    ...
    if get_lists_response2["status"] == 200:
        ...
        print_error(f"Failed to verify list count")
        return
